...
set_pane_split_bindings() {
  tmux bind-key "v"  run "$CURRENT_DIR/scripts/split.sh -h -c"
  ...
  tmux bind-key 's'  run "$CURRENT_DIR/scripts/split.sh -v -c"
  tmux bind-key "-"  run "$CURRENT_DIR/scripts/split.sh -fv -c"
}
